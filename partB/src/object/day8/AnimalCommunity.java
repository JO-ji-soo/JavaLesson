...
public class AnimalCommunity {
	...
	public static void main(String[] args)  {
		System.out.println("~~~~~여기는 유기동물 분양소입니다.~~~~~~");
			...
			//퀴즈 :  아래 객체를 배열 요소에 대입했습니다. 수정이 필요한 코드는?? =>break 대신 continue
	...
	//  동물보호소 규정(비지니스 로직)
	//					ㄴ 강아지는 15세이상, 고양이는 18세이상, 토끼는 13세 이상 
	//																			그 외는 20세 이상 분양받을 수 있습니다.		
	//          ㄴ Member 클래스 isAdopt 메소드에서 실행. ->instanceof 연산 사용	
		
		String name="모모";
		int age = 17;
		Member member = new Member(name, age);
	  //  Member member = new Member(age, name); //member클래스 인스턴스 필드는 age,name 2개.
		
		StringBuilder sb = new StringBuilder(name); //String은 문자열 연결결과 다른 문자열이 나오면 객체 새로 만들어 저장
		//String은 불변객체(문자열 변경 못함), 
		//StringBuilder는 가변객체(문자열 변경 가능. 문자열 추가연산결과를 동일 객체에 저장)=>문자열 연결연산에 사용
		sb.append(" 회원님 \n");					 	 //message +="회원님 \n";
		boolean result;
		for(int i=0;i<animals.length;i++) {
			if(animals[i]==null)
					continue;
			//break; ->첫번째로 null을 만나면 break | continue는 null만나면 다음 명령어 실행 안하고 for문 i++부터 다시 시작 //배열 요소에 참조값이 더이상 없으니 break 하기
			 // result =member.isAdopt(animals[i]);	//인자를 Animal타입으로 전달받는 메소드(다형성 예시)
			 sb.append(animals[i].toString());    //message += animals[i].toString()''
			if(member.isAdopt(animals[i])) {			//if(result) 대신에 직접 메소드 실행하고 리턴 받아 검사
				sb.append("  😃 분양 가능합니다. \n"); //message += "분양가능합니다. \n"
			} else {
		...
		System.out.println();
		System.out.println(sb); //StringBuilder 타입 객체는 println의 인자로 쓸때 toString 명시적 실행
