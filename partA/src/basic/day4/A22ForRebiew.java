...
//for문 복습 1~5번까지 구현. 6) 구구단 출력하기.
public class A22ForRebiew {public static void main(String[] args) {
    ...
    System.out.println(String.format("문제 1부터 %d 까지의 합계 : %d",  n, sum));
    System.out.println("문제 1부터"+n+"까지의 합계 : "+sum);
    
    // 2. 1부터 정수 n까지 곱하기
    n=13;          
    //int 타입은 정수 -2147483648 ~ +2147483647 그래서 곱센결과 큰 값이 나올것을 예상하면
    ...
    for(int i=1;i<=n;i++){
     ...
     for(int i=1;i<=n;i++){
        sum +=1;
        System.out.println(String.format("문제3  %d부터 %d까지의 더하기 : %,d", m, n, result));
