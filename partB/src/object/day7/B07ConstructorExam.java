...
public class B07ConstructorExam {
  public static void main(String[] args) {
    ...
    System.out.println("\tstr1 :" + str1);        // 길이가 0인 문자열 객체가 생성(출력)됨
    System.out.println("\tstr1 :" + str2);        // 
    System.out.println("\tstr1 :" + str3);        // 

    //2) 문자열 리터럴 특징 : 문자열 내용이 갚으면 또 메모리에 새로 저장하지 않습니다.
    //                      message가 참조하는 "Hello"와 동일한 리터럴을 temp가 참조할 때,
    //                      "Hello~"를 또 저장해서 temp를 참조하지 않고 이미 저장되어 있는 "Hello~" 참조
    System.out.println("message=temp ?" + (message==temp)); //참조값 비교. 그러므로 message의 temp는 기억장소 동일 => true
    System.out.println("message=str2 ?" + (message==str2)); //단, new연산으로 만든 것은 언제나 메모리 새로 할당 => false

    //3) 문자열 내용 비교 : equals 메소드
    System.out.println("message.equals(temp)?" +message.equals(temp)); //true
    ...
    String name = sc.nextLine();         //         문자열을 변환
