...
public class A27ArrayBasic {
	//괄호의 종류 : ()는 소괄호. 메소드에서 쓰임. {}는 중괄호. 블럭(영역)을 표시할 때 쓰임. []는 대괄호. 배열정리를 할 때 쓰임.(초기화할때는 중괄호)
	public static void main(String[] args) {
		...
		int[] arr2 = new int[3];		//선언방법2. 초기값은 기본값 0 , 정수 10개 (40바이트 할당), 인덱스 0~9
		
		//배열의 주요 속성 : length 는 배열의 길이(개수)를 갖습니다.
		System.out.println("배열의 크기(길이) 구하는 속성 length테스트");
		...
		for(int i=0;i<arr2.length;i++){ 	//arr2.length 대신에 3으로 고침.
			System.out.println("번째 값 입력 >>>");
			arr2[i] = sc.nextInt();
		}
		...
		printArray(arr1);   //메모리에 저장되는 데이터는 주소를 갖습니다. 
							//arr1,arr2는 배열의 이름=배열의 시작주소
